...
public class Circle {
    ...
    public enum Color {
        RED, BLACK, BLUE, YELLOW, GREEN, MAGENTA;

        public Color getRandom() {
            return Color.values()[ThreadLocalRandom.current().nextInt(0, Color.values().length)];
        }
    }
    ...
    private void draw() {
        if (isVisible) {
            ...
            canvas.draw(this, color.toString(), new Ellipse2D.Double(xPosition, yPosition,
                    diameter, diameter));
            canvas.wait(10);
        }
    }
